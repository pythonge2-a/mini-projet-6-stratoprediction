...
def input_launch_date():
    ...
    # while True:
    #     # Limite à 6 chiffres au total
    #     if len(resultat) >= 5:
    #         return resultat  # Arrête l'entrée après 6 chiffres
    ...
    #     # Si Enter est pressé
    #     if char == '\r' or char == '\n':
    #         return resultat  # Nouvelle ligne
...
""" A taper:
    latitude de départ (DMS:[--° --' --"])
    longitude de départ (DMS:[--° --' --"])
    altitude de départ (m)
    vitesse d'ascension (m/s):
    >0, 
    altitude d'explosion (m):
    >0, <~40000
    date de lancement (--.--.----):
    >now - 9 jours, <now+ 1jour
    heure de lancement (--h--m--s):
